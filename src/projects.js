  ...
  },
  {
    ...
    detail: {
      ...
      prompts: [
        {
          title: "(초안) RLS 정책 설계",
          text: "실제로 썼던 핵심 프롬프트로 바꿔 주세요. 예: 중개사무소(tenant)별로 데이터가 완전히 분리돼야 해. 모든 테이블에 office_id를 두고, 로그인한 유저의 office_id와 일치하는 행만 접근되도록 Supabase RLS 정책을 만들고, 정책 누락 시 데이터가 새는 케이스도 점검해줘.",
        },
        {
          title: "(초안) 실거래가 시세 조회",
          text: "실제로 썼던 핵심 프롬프트로 바꿔 주세요. 예: 국토부 실거래가 API로 매물 주변 시세를 가져와 캐싱하고, 데이터 출처를 표시하는 구조를 만들어줘.",
        },
      ],
      // 회고 — (초안) 직접 작성
      retro: [
