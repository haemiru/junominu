...
> **🎯 지금 곧바로 할 일 — ① 블로그 3편 초안 안내문 제거.**
> 각 글(`src/posts/*.md`) 상단 `> 이 글은 초안입니다…` 문장. **본문을 준 말투로 확정한 뒤 그 줄만 빼면 끝.** 현재 사이트(`/blog`)에 노출 중이라 우선순위 최상. 글 3편 = ① 첫 글 ② "데이터 한 곳만 고치면 화면이 따라온다" ③ "윈도우 대소문자 삽질기".

돌아오면 여기부터. 전부 **사용자 결정/내용**이 필요한 것들 — 코드 구조는 준비됨.

1. **📝 블로그 3편 초안 안내문 제거** *(← 지금 1순위)* — 각 글 상단 `> 이 글은 초안입니다…` 문장. 본문을 사용자 말투로 확정하면 그 줄만 빼면 됨. (현재 화면에 노출 중)
2. **⚠️ 추정 날짜 확정** — 짱샘·킁킁·i-talk의 `launched`(현재 추정). 실제 공개일 알면 교체.
3. **⚠️ 소리야 URL 임시** — `speech-therapy-nine.vercel.app/home`. 정식 URL 나오면 `projects.js` `url` 교체(+ 재캡처).
4. **(선택) 뉴스레터** — Phase 4 마지막. 도구(Buttondown/ConvertKit/자체) 선택만 하면 폼 연결 가능.
5. **(선택) 내부 스샷 추가** — 짱샘·중개프로는 현재 랜딩만. 로그인 후 내부 화면 주면 갤러리로 추가.

> ✅ **끝난 것**: 소리야 스택 검증(2026-06-04) — 실제 Next.js 15 확정(Vite 의심은 오판), summary/features 4게임 확장 완료. 자세한 내역은 아래 변경 로그.
