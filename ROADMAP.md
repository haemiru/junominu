...
- **콘텐츠/구조 자동 작업은 모두 소진.** 5개 프로젝트 상세 보유, 홈에 About·지표·Now·STACK·JOURNEY(최신순)·PROJECTS, 블로그(`/blog`, 글 2편), OG 이미지(`og.png`), sitemap·robots.
- **요약·기능·스택·타임라인·프롬프트 = 5개 프로젝트 전부 실데이터.** 상세 타임라인은 최근순 표시.
...
> **#3(프롬프트·막힌점·회고) 전 프로젝트 초안 0 — 전부 실데이터.** 남은 `⚠️`는 추정 날짜(launched/소리야 started)와 소리야 타임라인뿐.

**공개 전 남은 일:** ① 위 ⬜ 3개(중개프로·i-talk·소리야) 막힌점·회고 확정 ② `⚠️` 추정 날짜 확정 ③ ~~실제 스크린샷 PNG~~ — **5개 전부 완료**(짱샘·중개프로 랜딩 / 킁킁 5장·i-talk 2장 제품 화면 / 소리야 홈) ④ **⚠️ 소리야 URL 임시**(`speech-therapy-nine.vercel.app/home` — 추후 변경 예정. 옛 `...ten-theta...`는 404로 폐기) ⑤ (선택) 뉴스레터 도구 선택 ⑥ (선택) 블로그 글 다듬기.
