...
**다섯 번 갈아엎고 나온 결론이다. 되돌리기 전에 반드시 읽을 것.**

| 시도 | 결과 |
|---|---|
| ① 썸네일 9개 가로 스트립 | 모바일은 괜찮은데 **PC 폭에서 줄줄이 늘어서** 나빴다 |
| ② 화면 한 장만 크게 | **"junominu.com 이 중개프로 랜딩"** 처럼 보였다 — 포트폴리오인데 |
| ③ 큰 것 하나 + 작은 것 둘 | 나아졌지만 **여전히 특정 프로젝트 진열**이었다 |
| ④ **아예 뺐다** ✅ | **간판은 상품 진열장이 아니다.** 프로젝트는 바로 아래 PROJECTS 에서 본다 |

대신 **이 사이트에만 있는 것**을 놓는다 — `ME.contrast` 의 **「IT 회사 22년 동안 0개 → AI 바이브 코딩 N개월 동안 N개」** 낙차.
스레드 계정 고정글의 반전 훅과 같은 메시지라, **글 보고 온 사람이 같은 이야기를 다시 만난다.** 오른쪽 숫자는 `ME.since` 와 `PROJECTS` 길이로 자동 계산된다.
