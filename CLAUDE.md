...
- 히어로(`.herowrap`)와 상단 바는 **카드가 아니라 전면**이라 흰색이다.
...
#### 히어로·풀블리드 구현 메모 (건드리기 전에 읽을 것)

- **홈은 `.page` 한 덩어리가 아니다.** `topbar` → `.page--top`(히어로) → `.strip`·`.nums`(풀블리드) → `.page--rest` 로 **형제**다.
  🔴 풀블리드를 `.page` 안에 넣고 `margin: 0 calc(50% - 50vw)` 로 빼면 **50vw가 스크롤바 폭을 포함해 가로 스크롤이 8px 생긴다**(2026-08-03 실측). 구조로 푼 이유가 이것이다.
- **`.strip` 은 `overflow-x: auto`** — `hidden` 으로 두면 좁은 화면에서 뒤쪽 프로젝트를 영영 못 본다. 스크롤바만 감춘다.
