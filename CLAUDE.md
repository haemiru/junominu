...
#### 히어로 아래는 "대표 화면 한 장"이다 (썸네일 줄 아님)

`ME.heroShot` 한 장을 크게 놓는다(`ME.heroShotSlug`·`heroShotAlt` 로 링크·대체텍스트 지정, 없으면 대표작 첫 번째).
🔴 **썸네일을 가로로 늘어놓는 방식(`.strip`)은 폐기했다** — 모바일은 괜찮아도 PC 폭에서 작은 카드가 줄줄이 늘어서 보기 나빴다(2026-08-03). "여러 개"라는 사실은 바로 아래 숫자 밴드가 말한다.
