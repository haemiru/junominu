...
- **블로그 글쓰기**: `.claude/skills/blog-post/SKILL.md` — 프로젝트 1개 → 글 1편. 분량·이미지·FAQ·검증 규약이 전부 여기 있다. 짱샘의 책방 `/add-blog-and-reviews-to-ebook`을 이 사이트(정적 md·앱 캡처·짧은 분량)에 맞게 옮긴 것.
  - 🔴 **재료는 `detail` + 라이브 화면 두 곳이고, 어긋나면 라이브가 이긴다**(2026-08-14 개정 — 아래 2편 스킬과 같은 규칙).
- **블로그 2편 묶음**: `.claude/skills/add-blogs-to-project/SKILL.md` — 프로젝트 1개 → 글 **2편**. 같은 원본 스킬(책방 3편+후기)에서 나왔지만 핵심이 다르다: **글을 쓰기 전에 재료를 2등분하는 단계**가 있고, 두 글의 각도·주 키워드·첫 이미지·hook 패턴이 겹치지 않게 강제하며 서로를 링크한다. 재료가 2편을 못 채우면 1편만 쓰고 그렇게 보고한다. 후기·댓글 생성은 없다.
  - 🔴 **재료는 `detail` + 라이브 화면 두 곳이고, 어긋나면 라이브가 이긴다**(2026-08-14 개정). `detail`은 손으로 적는 요약이라 제품이 움직이면 곧바로 낡는다 — 중개프로는 요금제가, 아이복지모아는 치료실 수(2,270 → 5,921)와 무료 운영 구조가 `detail`에 없거나 틀려서 **글의 뼈대를 라이브에서 가져와야 했다.** 어긋난 항목은 「낡은 값」 표로 보고하므로, 받으면 `projects.js`를 고칠 것.
