...
#### 히어로 아래는 "큰 것 하나 + 작은 것 둘" (비대칭 3분할)

**세 번 갈아엎고 나온 답이다. 개수를 바꾸기 전에 이 이력을 읽을 것.**

| 시도 | 결과 |
|---|---|
| ① 썸네일 9개를 가로 스트립으로 | 모바일은 괜찮은데 **PC 폭에서 작은 카드가 줄줄이 늘어서** 나빴다 |
| ② 한 장만 크게 | 이번엔 **"junominu.com 이 중개프로 랜딩"** 처럼 보였다 — 포트폴리오인데 |
| ③ **셋** ✅ | 하나면 그 제품 사이트가 되고, 아홉이면 잡동사니가 된다. **셋이 그 사이** |

- 큰 자리는 `ME.heroShot`(+`heroShotSlug`·`heroShotAlt`), 나머지 둘은 대표작에서 자동으로 채워진다.
- 높이는 **그리드의 `aspect-ratio`** 가 정하고 행이 그걸 반씩 나눈다(자식 aspect-ratio 로 하면 순환 계산이 된다).
- `object-position: **left top**` — 웹 화면은 좌상단(로고·내비)이 정보다. `center` 로 두면 로고가 잘린다.
