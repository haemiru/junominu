...
#### 색 리듬 — 흰/그레이만 반복하지 않는다

흰색↔그레이만 오가니 밋밋하다는 지적(2026-08-03). 색이 들어가는 자리를 **두 곳만** 정했다.

```
히어로      흰 + 점격자 + 파란 광
PROJECTS   흰
프롬프트노트 ▓ 옅은 파랑 (--accent-wash)
ABOUT      ▓ 그레이 (--surface-sunken)
NOW        흰
JOURNEY    ▓ 그레이
함께하기    ██ 먹색 #0c1626  ← 페이지에서 유일한 진한 블록
풋터        흰
```

- **먹색 CTA가 색 리듬의 핵심**이다. 마지막 전환 지점이라 시선이 가야 하는 자리고, 여기 하나로 페이지 전체 인상이 바뀐다. 먹색은 `DESIGN.md` §4 `marketing-dark rgba(0,12,30,.8)` 계열.
- 먹색 위에서는 **파란 버튼보다 흰 버튼이 세다**(`.cta .btn--primary`).
- `.cta` 는 `.page` 의 **형제**다(풀블리드). 안쪽 폭은 스스로 잡는다.
- 🚫 **라디얼 그라데이션을 카드 배경에 겹치지 말 것** — `.band`·`.cta` 가 그랬는데 **얼룩져 보였다.** 단색 면으로 간다.
