...
### 🔗 중개프로 링크는 이제 **`jungaepro.com`**(제품 랜딩) — 서브도메인으로 보내지 말 것
서브도메인(`jungae.jungaepro.com` 등)은 **입점 사무소의 손님 페이지**다. 콜드 방문자를 보내면 안 된다.
스레드 CTA·`projects.js`·앞으로 쓸 글 전부 **`jungaepro.com`** 으로 통일한다(사용자 지시 2026-08-02).
옛 주소 `smart-sjhome.vercel.app` 은 폐기 — `drafts/`·`threads-log.md` 의 옛 UTM 링크는 **과거 기록이라 그대로 둔다**(새 글에서 복사하지 말 것).

> ⚠️ **남은 버그(우선순위 낮음, `jungaepro` 레포)**: 테넌트 상단 메뉴 8개 중 **홈만 살아있고 7개가 404**.
> 원인은 GNB 메뉴 목록이 목업이라는 것 — `src/utils/mockData.ts:197` `gnbMenuItems` 가 `UserGNB.tsx:34/117` 에서 그대로 `<NavLink>` 로 렌더된다.
> 특히 **`/market-price` 는 라우터의 `market-info` 오타** — 페이지는 멀쩡히 살아 있다(실거래가 차트·신호등·평당가 표 확인). **한 단어만 고치면 부활.**
> 나머지 6개(`매물`·`지도`·`급매추천`·`분양정보`·`부동산핫이슈`·`중개사무소찾기`)는 페이지 자체가 없다 → 메뉴에서 빼거나 만들어야 함.
> 홍보 링크가 랜딩으로 바뀌어 **퍼널에는 더 이상 영향 없음.** 입점 사무소 손님에게만 보인다.

> ✅ **projects.js 갱신 완료(2026-08-02)** — 아이복지모아 보조기기센터 **41곳**·커뮤니티·치료실 계정 추가, 중개프로 `url` → `jungaepro.com`.
...
- 07-27~29에 **8편** 다 나감(중개프로 매물장/계약서/CRM · 아이복지모아 치료실/포스터/계보/보조기기센터 · 기억정원 회상치료). `drafts/`에 미발행 초안 **없음**.
