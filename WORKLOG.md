...
## ▶ 지금 할 차례 (2026-07-27 기준)

> **"이제 뭐 해야 해?" → 사이트 코드는 다 됐다. 지금 성장 레버는 세 개: ① 스레드 글 발행(대기 중 1편) ② 답글 세션(팔로워) ③ 블로그 콘텐츠(심화 경로).**

1순위(라우트별 메타·OG 프리렌더)·2순위(홈 첫인상)·3순위(번들 다이어트) 모두 **완료**. 사이트 코드 쪽 대기열은 비었고, 남은 건 **콘텐츠 + 채널 운영**이다.

### 🧵 스레드 — 07-27 **하루 3편 발행 → 팔로워 2배(28→55)** 🎉
- ✅ 중개프로(매물장 PDF) · ✅ 아이복지모아(치료실 2,270곳 지도) · ✅ 아이복지모아(게시판 포스터).
- 🏆 **치료실 지도 글이 터졌다** — 좋아요 **149** · 답글 10 · **리포스트 17** · 인용 10. 최근 조회 8,905 → **1.8만**. `threads-log.md` 아웃라이어 표에 지정 → **8월 중순 각도 바꿔 재활용**.
- 📌 **발행 리듬 방침 변경(사용자 지시 07-27)**: *"지난 글이 좀 묻히더라도 주기적으로 계속 올리다 보면 계속 반응이 있다. 터진 글은 나중에 리마인드로 다시 올리면 된다."*
  - → SKILL §9 **"하루 1개 상한" 폐기**. **Claude는 발행을 미루라고 조언하지 않는다.** 초안이 준비되면 바로 낸다. (28분 간격 2편이 묻히지 않고 오히려 터진 게 근거)
- **지금 할 일**: 답글 즉답. **답글에 달리는 게 다음 글감**(중개프로는 "매물 설명 어떻게 하시냐"를 3지선다로 물어놨음).
- 📌 **다음 글감(로그에 대기 중)**: 중개프로=계약서 버튼 하나·CRM·손님 시점 / 아이복지모아=짱샘의 책방 자매 서비스·아이가 화자. **마감·알림톡 각도는 템플릿 심사 통과 후에.**
- ⚠️ **[연재 2화] 미이행** — 07-23 연재 1화에서 *"출시하면 첫 결제 결과를 올리겠다"* 고 약속했다. 정식 출시 후 반드시 쓸 것.
- ⚠️ **[연재 2화] 미이행** — 07-23 연재 1화에서 *"출시하면 첫 결제 결과를 올리겠다"* 고 약속했다. 정식 출시 후 반드시 쓸 것(안 쓰면 예고 전략 자체가 죽는다).

### 🗣️ 팔로워 레버 — 스레드 답글 엔진 (7회차까지 완료)
- **`/threads-reply` 스킬**로 "댓글 써줘" 한마디면 타깃 검색 → 자가검증 답글 → 승인 카드 → 발행이 돈다. 캡처를 주면 **대댓글 추천안 3개**(모드 B).
- **누적 36개 계정**(`reply-log.md`) — 스캔 때 자동 제외되므로 재답글 걱정 없음. **주 5일 리듬 유지**가 전부다.
- ✅ **발행 절차는 7회차에서 굳었다**(`card-ui.md`): 전송 버튼 `svg[aria-label="답글"]` · `fill`+`send` 한 호출(`window.go`) · **한글 이스케이프 금지**(오타 3건 사고) · 발행 후 승인본 대조.
