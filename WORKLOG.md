...
> **"이제 뭐 해야 해?" → 사이트 개선 3순위 「번들 다이어트」부터.**

1순위(라우트별 메타·OG 프리렌더)·2순위(홈 첫인상) 모두 **완료**. 다음은 아래 순서.

### 🟡 3순위 — 번들 다이어트 ← **여기부터**
- 354KB(gzip 116KB). `marked`(마크다운 파서)가 **블로그에서만 필요한데 전 페이지 번들에 포함**돼 있다. `React.lazy` + 동적 import로 `/blog` 계열만 로드하도록 분리.
