...
### 🗣️ 팔로워 레버 — 스레드 답글 엔진 (**9회차까지 완료**)
- **`/threads-reply` 스킬**로 "댓글 써줘" 한마디면 타깃 검색 → 자가검증 답글 → 승인 카드 → 발행이 돈다. 캡처를 주면 **대댓글 추천안 3개**(모드 B).
- **누적 52개 계정**(`reply-log.md`) — 스캔 때 자동 제외되므로 재답글 걱정 없음. **리듬 유지가 전부다.**
- ✅ **발행 절차 확정**(`card-ui.md`): 전송 버튼 `svg[aria-label="답글"]` · `fill`+`send` 한 호출(`window.go`) · 🚨 **한글 이스케이프 금지**(7·8회차 오타 사고 → 9회차 0건) · 발행 후 승인본 대조 · 수정 모달은 `[role="dialog"]` 안에서 선택.
- 📉 **타깃 고갈 조짐**: 9회차 공인중개사 검색은 **매물 광고가 대부분**이라 2건만 건졌다. 검색어를 넓히거나(`임장`·`중개사무소`) 니치 배분을 조정할 시점.
