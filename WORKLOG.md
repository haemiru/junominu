...
### 🔴 중개프로 랜딩 상단 메뉴가 전부 404다 (미해결 · 사용자 결정 대기)
`/market-price`·`/properties`·`/map`·`/urgent`·`/presale`·`/find-agent`·`/consultation` — **두 도메인 모두 동일. 홈만 정상.**
서버는 200(SPA index.html)을 주는데 라우터에 그 경로가 없다. **스레드 홍보 글 3편이 이미 이 사이트를 가리키고 있어서**, 글 보고 온 사람이 메뉴를 누르면 404를 만난다.

> ✅ **projects.js 갱신은 완료(2026-08-02)** — 아이복지모아 보조기기센터 **41곳**·커뮤니티·치료실 계정 추가, 중개프로 `url`을 `jungae.jungaepro.com` 으로 교체.
