...
### 🔧 projects.js 갱신 대기 (발행글에 이미 공개된 내용)
- **아이복지모아**: ① **보조기기센터 전국 41곳** ② **커뮤니티 기능** — 07-29 발행글에 둘 다 나갔는데 `features`에 없다.
- **중개프로**: `url`이 옛 주소(`smart-sjhome.vercel.app`). 실제는 **`jungae.jungaepro.com`**(멀티테넌트 서브도메인).
- ⚠️ **중개프로 랜딩 상단 메뉴 전부 404** — `/market-price`·`/properties`·`/map`·`/urgent`·`/presale`·`/find-agent`·`/consultation`. 두 도메인 모두 동일. 홈만 정상. 홍보 글 보고 온 사람이 메뉴를 누르면 404를 만난다.
