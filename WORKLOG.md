...
### 🧵 스레드 — 07-27 밤 2편 발행 완료, 지금은 **골든타임 관리**
- ✅ **중개프로**(매물장 PDF 한 장) · ✅ **아이복지모아**(치료실 2,270곳 지도) 둘 다 발행. 팔로워 27→**28**, 최근 조회 8,905.
- **지금 할 일**: 답글이 달리면 60~90분 안에 즉답. **답글에 뭐가 달리는지가 다음 글감**이다(중개프로는 "매물 설명 어떻게 하시냐"를 3지선다로 물어놨음).
- ⚠️ **28분 간격으로 두 편을 올렸다** — §9 하루 1개 상한 위반. 새 글이 이전 글의 답글 누적을 끊는다. **다음엔 하루씩 뗄 것.**
- 📝 **발행 대기 1편**: 아이복지모아 **게시판 포스터** `drafts/2026-07-27-bokjimoa-G-poster.txt` (93/100). voice-profile **규칙 24~30 첫 적용작**. ⏰ 오늘 이미 2편 나갔으니 **내일** 올릴 것.
