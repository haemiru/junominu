...
### 현재 상태 ✅
- 문서 3종 정합성 확인(낡은 수치 잔존 없음). 코드 변경 없음 → 사이트 영향 없음.

### 다음에 할 일 (TODO)
- [ ] **Vercel Analytics 설치 + 링크 UTM** — ⚠️ 지금 junominu.com에 계측이 **아예 없어** 스레드→사이트→문의 경로가 측정 불가. 이게 없으면 아웃라이어 재활용(전략 §8)의 근거가 안 나옴. **최우선.**
