...
**① 기억정원 — 사진을 실제로 어디에 저장하나** *(Claude 가 직접 확인 가능)*
07-28 발행글엔 **"클라우드로 안 보냅니다 · 로컬에서만"**, `projects.js` 엔 **"비공개 버킷 + 서명URL"**(= 클라우드). **정반대다.**
**공개적으로 개인정보 처리를 말한 것**이라 둘 중 뭐가 맞는지 확인해야 한다.
→ `D:\Claude-prj\dementia-prevention` 코드를 열어보면 알 수 있다. **"봐드릴까요?" 하고 물을 것.**
→ 맞추는 순서: **실제 구현 → 약관/개인정보처리방침 → `projects.js`**

**② 중개프로 상단 메뉴 7개 404** *(사용자 결정만 필요)*
`jungaepro` 레포 일이고, 링크를 `jungaepro.com` 으로 바꿔서 **퍼널엔 영향 없다.** 입점 사무소 손님에게만 보인다.
- `market-price` → `market-info` **한 단어**만 고치면 시세 페이지는 바로 부활(페이지는 멀쩡함)
- 나머지 6개는 페이지가 없어서 **메뉴에서 빼야** 한다
...
### 🔗 중개프로 링크는 **`jungaepro.com`**(제품 랜딩) — 서브도메인으로 보내지 말 것
서브도메인(`jungae.jungaepro.com` 등)은 **입점 사무소의 손님 페이지**다. 콜드 방문자를 보내면 안 된다.
스레드 CTA·`projects.js`·앞으로 쓸 글 전부 `jungaepro.com` 으로 통일(사용자 지시 2026-08-02).
옛 주소 `smart-sjhome.vercel.app` 은 폐기 — `drafts/`·`threads-log.md` 의 옛 UTM 링크는 과거 기록이라 그대로 둔다(새 글에서 복사 금지).

> ⚠️ **남은 버그(우선순위 낮음, `jungaepro` 레포)**: 테넌트 상단 메뉴 8개 중 **홈만 살아있고 7개가 404**.
> `src/utils/mockData.ts:197` `gnbMenuItems`(목업)가 `UserGNB.tsx:34/117` 에서 그대로 `<NavLink>` 로 렌더된다.
> 특히 **`/market-price` 는 라우터의 `market-info` 오타** — 페이지는 멀쩡하다(실거래가 차트·신호등·평당가 표 확인). **한 단어만 고치면 부활.**
> 나머지 6개는 페이지 자체가 없다 → 메뉴에서 빼거나 만들 것. 홍보 링크가 랜딩으로 바뀌어 **퍼널에는 영향 없음.**
