...
## ▶ 지금 할 차례 (2026-07-25 기준)

> **"이제 뭐 해야 해?" → 사이트 코드는 다 됐다. 지금 성장 레버는 두 개: ① 스레드 답글 세션(팔로워) ② 블로그 콘텐츠(심화 경로).**

1순위(라우트별 메타·OG 프리렌더)·2순위(홈 첫인상)·3순위(번들 다이어트) 모두 **완료**. 사이트 코드 쪽 대기열은 비었고, 남은 건 **콘텐츠 + 채널 운영**이다.

### 🧵 팔로워 레버 — 스레드 답글 엔진 ← **여기가 지금 제일 효과**
- **`/threads-reply` 스킬**(2026-07-25 신설)로 "댓글 달아줘" 한마디면 타깃 검색 → 자가검증 답글 → 승인 카드 → 발행이 돈다. 조회는 나오는데 팔로워가 안 느는 이유 = 답글 0건이었음(전략 §5).
- **지금 당장**: ① 미발행 2개(cholmin0318·exportengine) 붙여넣기 ② **답글 세션을 주 5일 리듬으로**(동료·큰계정·타깃 3개 링). 이미 답글 단 계정은 `reply-log.md`에 있어 재답글 안 함.
- ⚠️ 발행 시 렌더러 글리치(띄어쓰기 유실) 있음 → 스킬이 **게시 직전 JS 검증**으로 잡는다. 새 탭에서 하면 덜 샌다(`card-ui.md`).
