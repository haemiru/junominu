...
### 🧵 발행 대기 — 스레드 초안 2편 ← **여기부터**
- **중개프로** `drafts/2026-07-27-jungaepro-G-report.txt` (91/100). 07-24 발행글의 다음 회차 — 같은 공인중개사분, 이번 불편은 「손님한테 설명이 안 남는다」. 토픽 `#공인중개사`, `utm_campaign=jungaepro-report`.
- **아이복지모아** `drafts/2026-07-27-bokjimoa-F-therapymap.txt` (92/100). 07-26 발행글의 다음 단계 — 「받을 수 있게 됐는데 갈 데를 못 찾는다」(치료실 2,270곳 지도). 토픽 `#느린아이`, `utm_campaign=bokjimoa-therapy-map`.
- ⚠️ **하루 1개 상한**(§9) — 새 글이 이전 글의 답글 누적을 끊는다. **하루씩 떼서** 올릴 것.
- 발행하면 `threads-log.md` 상태를 `발행`으로 바꾸고 반응을 채운다. **답글에 달리는 게 다음 글감.**
...
**⑤ 중개프로 스레드 글 초안 1편** — `drafts/2026-07-27-jungaepro-G-report.txt` (채점 91/100). 07-24 발행글(통합플랫폼)의 **다음 회차**로, 같은 공인중개사분의 불편을 한 단계 좁혔다(하루 전체 → 손님한테 설명이 안 남는다). 훅은 발행글에서 처음 쓰는 **상식뒤집기형**. 미발행.
...
### ⏭️ 다음
- **중개프로 G 발행** → 로그 상태 `발행`으로 바꾸고 반응 채우기. 답글에 뭐가 달리는지가 다음 글감.
