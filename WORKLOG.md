...
**검증**: 생성 15개(프로젝트 8 · 글 3 · 고정 3 + 루트) · 참조된 shot 8개 전부 실존 · 중첩 폴더에서도 에셋 절대경로 정상 · width/height 제거/유지 분기 정상.

### 현재 상태
- 배포 후 라이브에서 라우트별 메타 실측 확인 필요(Vercel 파일 우선 서빙 동작 확인).

### 다음에 할 일 (TODO)
- [ ] 사이트 개선 2순위: **홈 첫인상** — 히어로 CTA가 "1:1 코칭·외주 문의" 하나뿐이라 문턱이 높음. "프로젝트 둘러보기" 같은 낮은 문턱 CTA 병기 + `/prompts`(5개 프로젝트 실제 프롬프트) 노출 강화.
- [ ] 3순위: **번들 다이어트** — 347KB(gzip 113KB). `marked`가 블로그에서만 필요한데 전 페이지 번들에 포함 → 코드 스플리팅.
- [ ] 4순위: **블로그** — 글 3개, 마지막 2026-06-04. 스레드 🔧빌드로그·💡인사이트를 받는 심화 글 경로가 비어 있음.
