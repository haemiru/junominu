...
description: >-
  ...
  받아 detail 실데이터(타임라인·막힌 점·프롬프트·회고)와 라이브 화면을 함께 읽고
  — 둘이 어긋나면 라이브가 이긴다 — 2,000~3,000자 SEO 글을 쓰고,
...
- **원재료는 두 곳이다 — `src/projects.js`의 `detail` + 라이브 화면. 🔴 어긋나면 라이브가 이긴다**(단계 1). 날짜·기능·막힌 점·가격을 **지어내지 말 것**. 둘 다 없으면 사용자에게 묻는다.
- **어긋난 항목은 「낡은 값」으로 모아 단계 7에서 보고한다** — `detail`을 고치는 건 사용자 몫이지만, 알려주지 않으면 다음 글도 같은 값으로 틀린다.
...
2. `detail`에서 다음을 뽑아 메모한다 — **이게 글의 뼈대다**:
   ...
   - `retro` → 회고
3. 🔴 **`url`이 있으면 라이브 화면을 캡처해서 읽는다.** 단계 3에서 어차피 이미지를 떠야 하니 순서만 앞당기는 것이다.
   전체 페이지를 길게 뜨고(`--window-size=1280,7000` 정도), 필요하면 잘라 확대해 문구·숫자·요금을 **눈으로 읽는다.**
4. **`detail`과 라이브가 어긋나면 라이브를 쓴다.** `detail`은 **적어 둔 시점의 요약**이고 라이브는 **지금**이다.
   어긋난 항목은 「낡은 값」 목록에 적어 두었다가 **단계 7에서 보고**한다.
5. `detail`도 없고 라이브도 못 열면 글을 쓸 재료가 없다 → 사용자에게 알리고 중단하거나, 재료를 받아온다.
6. `src/posts/`의 기존 글 제목·slug를 확인해 **중복 각도 회피**.

> **왜 라이브가 이기나 — 2026-08-14에 두 번 연속 같은 일이 났다**(자매 스킬 `/add-blogs-to-project` 실행 중).
>
> | 프로젝트 | `detail` | 라이브 |
> |---|---|---|
> | 중개프로 | 요금제·기능 구성이 **아예 없음** | Free·14,900·29,900원, 9개 기능, 가입 3단계 |
> | 아이복지모아 | 치료실 **2,270곳** | **5,921곳** (보조기기센터는 검색에 흡수) |
> | 아이복지모아 | 무료 운영 구조 **기록 없음** | 홈·풋터에 "짱샘의 책방 수익으로 운영" 명시 |
>
> 두 번 다 **글의 뼈대가 `detail`에 없고 화면에만 있었다.** `detail`은 사람이 손으로 적는 요약이라 제품이 움직이면 바로 낡는다.
>
> ⚠️ 대신 **라이브에서 읽은 값(가격·개수)은 변한다.** 글에 박았으면 단계 7에서 보고해, 값이 바뀔 때 어느 글을 고쳐야 하는지 남긴다.
...
- 🔴 **「낡은 값」 표 (단계 1에서 모은 것)** — `detail`과 라이브가 어긋난 항목. 없으면 "없음"이라고 적는다.
  ...
  덧붙여 **라이브에서 읽어 글에 박은 값(가격·개수·요금제)** 을 나열한다 — 그 값이 바뀌면 이 글도 고쳐야 하기 때문이다.
...
- **`detail`과 라이브가 어긋나면 라이브를 쓰되, 조용히 넘어가지 않는다.** 어긋난 항목은 단계 7에서 보고한다(단계 1).
