...
## 🔴 크론이 발동해도 발행이 안 될 수 있다 (2026-08-09 실측)

08-09 08:10 크론이 **목록에서 사라졌는데(one-shot 은 발동하면 자동 삭제) 글은 안 나갔다.**
크론은 REPL 이 유휴일 때만 실행되고, 그 사이 무슨 이유로든 턴이 소비되면 발행 없이 지나간다.

- **크론 목록에 없다 = 발행됐다** 가 **아니다.** 판단 기준은 **`PUBLISHED.md` 기록과 프로필 확인**뿐이다.
- 특히 **밤을 넘긴 첫 슬롯(아침 08:10)** 이 위험하다. 사용자가 아침에 한 번 물어봐 주는 게 제일 확실하다.
- 늦게 발견하면 **그 시각에 바로 발행하고**, `PUBLISHED.md` 에 예정 시각과 실제 시각을 **둘 다** 적는다.
...
🔴 **후보가 여러 개면 「새 태그 생성」을 고르지 말 것.** 예: `바이브코딩` 을 치면
맨 위에 `바이브코딩` + `새로운 주제 태그하기` 가 뜨는데 이건 **아무도 안 보는 새 태그**다.
계정이 이미 구독 중이고 트래픽이 있는 **`바이브코딩 vibe coding`** 을 고른다.
`공인중개사` 는 맨 위 항목이 곧 기존 토픽이라 그대로 고르면 된다.
...
⚠️ **`Runtime.evaluate` 타임아웃이 나도 JS 는 실행돼 있을 수 있다**(2026-08-08 [4] 실측 — 45초 타임아웃이 났는데 본문은 정상 입력돼 있었다).
**바로 재시도하면 같은 글이 두 번 붙는다.** 타임아웃이 나면 먼저 가벼운 호출로 작성창 상태를 읽고,
필요하면 **게시 직전에 본문을 기대값과 문자열 비교**한 뒤 누른다.
